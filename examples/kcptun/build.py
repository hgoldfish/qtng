...
# -*- coding: utf-8 -*-
"""
kcptun Alpine/musl 静态构建脚本。

在 Alpine Linux 容器中用 musl 静态链接编译 kcptun，产出不依赖 glibc、
可在任意同架构 Linux 上运行的可执行文件。

用法:
  ./build.py [--no-cache]
      构建 Docker 镜像并编译 kcptun-server / kcptun-client，复制到 dist/
"""
...
IMAGE_NAME = "kcptun-musl-static"
DOCKERFILE_HASH_LABEL = "kcptun.dockerfile_hash"
KCPTUN_ROOT = Path(__file__).resolve().parent
QTNG_ROOT = KCPTUN_ROOT.parent.parent
DOCKERFILE_PATH = KCPTUN_ROOT / "Dockerfile"
DOCKER_BUILD_DIR = KCPTUN_ROOT / ".build-in-docker"
DIST_DIR = KCPTUN_ROOT / "dist"
ARTIFACTS = ("kcptun-server", "kcptun-client")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="kcptun Alpine/musl 静态构建")
    parser.add_argument(
        "--no-cache",
        action="store_true",
