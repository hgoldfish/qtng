...
{
    vector<V> values;
    const auto range = map.equal_range(key);  // pair of multimap<K,V>::const_iterator (types depend on the template args)
...
{
    vector<K> keys;
    for (const auto &entry : map) {  // entry is pair<const K, V>; K/V are template args
