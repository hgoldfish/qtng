...
extern "C" intptr_t BOOST_CONTEXT_CALLDECL jump_fcontext(fcontext_t *ofc, fcontext_t nfc, intptr_t vp,
                                                         bool preserve_fpu);
extern "C" fcontext_t BOOST_CONTEXT_CALLDECL make_fcontext(void *sp, size_t size, void (*fn)(intptr_t));
...
extern "C" void run_stub(intptr_t data)
{
    ...
    if (!coroutine) {
        ngDebug() << "run_stub got invalid coroutine.";
        return;
    }
...
BaseCoroutinePrivate::~BaseCoroutinePrivate()
{
    ...
    if (state == BaseCoroutine::Started) {
        if (q->objectName().empty()) {
            ngWarning() << "do not delete running BaseCoroutine:" << q;
        } else if (q->objectName().find("eventloop_coroutine") == string::npos) {
            ngWarning() << "do not delete running BaseCoroutine:" << q->objectName();
        }
    }
    if (exception) {
...
bool BaseCoroutinePrivate::yield()
{
    ...
    if (!result && state != BaseCoroutine::Stopped) {  // last coroutine private.
        ngWarning() << "jump_fcontext() return error.";
        return false;
    }
...
bool BaseCoroutinePrivate::initContext()
{
    ...
    if (!stackSize) {
        ngDebug() << "is the main fiber forgot to create context?";
        return true;
    }
    ...
    if (!context) {
        ngWarning() << "Coroutine can not malloc new context.";
        bad = true;
        return false;
    }
    return true;
