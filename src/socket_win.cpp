...
typedef INT (WSAAPI *LPFN_WSASENDMSG)(SOCKET s, LPWSAMSG lpMsg, DWORD dwint,
                                      LPDWORD lpNumberOfBytesSent,
                                      LPWSAOVERLAPPED lpOverlapped,
                                      LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
...
*/
static string qt_prettyDebug(const char *data, int len, int maxLength)
{
    if (!data) return "(null)";
    string out;
    for (int i = 0; i < len; ++i) {
        ...
        default:
            string tmp;
            char buf[16];
            snprintf(buf, sizeof(buf), "\\%o", static_cast<unsigned char>(c));
            tmp = buf;
            out += tmp;
        }
    }
...
*/
static inline void qt_socket_getPortAndAddress(SOCKET socketDescriptor, const qt_sockaddr *sa, uint16_t *port, HostAddress *address)
{
    if (sa->a.sa_family == AF_INET6) {
        ...
        ngWarning() << "qt_socket_getPortAndAddress can only handle AF_INET6 and AF_INET";
    }
}
...
static inline Socket::SocketType qt_socket_getType(intptr_t socketDescriptor)
{
    int value = 0;
    NG_SOCKLEN_T valueSize = sizeof(value);
...
namespace {
namespace SetSALen {
    template <typename T> void set(T *sa, typename enable_if<(&T::sa_len, true), int>::type len)
    { sa->sa_len = len; }
    template <typename T> void set(T *sin6, typename enable_if<(&T::sin6_len, true), int>::type len)
    { sin6->sin6_len = len; }
...
//        sendmsg = 0;
    ...
    fd = static_cast<intptr_t>(socket);
    if (socket == INVALID_SOCKET) {
        return false;
    } else {
        if(!setNonblocking()) {
...
bool SocketPrivate::bind(const HostAddress &a, uint16_t port, Socket::BindMode mode)
{
    (void)(mode);
    if (!checkState())  {
        return false;
...
#if defined (SOCKET_DEBUG)
        ngDebug() << "SocketPrivate::bind(%s, %i) == false (%s)",
               address.toString().get(), port, errorString.data());
#endif
        return false;
    }

#if defined (SOCKET_DEBUG)
    ngDebug() << "SocketPrivate::bind(%s, %i) == true",
           address.toString().get(), port);
#endif
...
bool SocketPrivate::listen(int backlog)
{
    ...
    #if defined (SOCKET_DEBUG)
            ngDebug() << "SocketPrivate::listen(%i) == false (%s)",
                   backlog, errorString.data());
    #endif
        return false;
    }
...
bool SocketPrivate::fetchConnectionParameters()
{
    ...
    if (this->protocol == HostAddress::IPv6Protocol) {
        bool ok = false;
        const uint32_t localIPv4 = localAddress.toIPv4Address(&ok);
...
#if defined (SOCKET_DEBUG)
    string socketProtocolStr = "UnknownProtocol";
    ...
    string socketTypeStr = "UnknownSocketType";
...
int32_t SocketPrivate::recv(char *data, int32_t size, bool all)
{
    ...
    while (total < size) {
        ...
        WSABUF buf;
        buf.buf = data + total;
        buf.len = static_cast<uint32_t>(size - total);
        DWORD flags = 0;
        DWORD bytesRead = 0;
        if (::WSARecv(static_cast<SOCKET>(fd), &buf, 1, &bytesRead, &flags, nullptr, nullptr) ==  SOCKET_ERROR) {
            int err = WSAGetLastError();
...
int32_t SocketPrivate::send(const char *data, int32_t size, bool all)
{
    ...
    ScopedIoWatcher watcher(EventLoopCoroutine::Write, fd);
    int32_t ret = 0;
    int32_t bytesToSend = min<int32_t>(49152, size);
    while (bytesToSend > 0) {
        if (!checkState()) {
            return ret == 0 ? -1: ret;
        }
        ...
        ret += bytesWritten;
        bytesToSend = min<int32_t>(49152, size - ret);
...
int32_t SocketPrivate::recvfrom(char *data, int32_t size, HostAddress *addr, uint16_t *port)
{
    if (!checkState() || size < 0) {
        return -1;
    }
    ...
    buf.buf = size ? data : &c;
    buf.len = size ? static_cast<uint32_t>(size) : 1;
    msg.lpBuffers = &buf;
    msg.dwBufferCount = 1;
...
//        }
        if (ret == SOCKET_ERROR) {
            ...
            if (err == WSAEMSGSIZE) {
                ...
                ret = min(size, static_cast<int32_t>(bytesRead));
            } else {
                WS_ERROR_DEBUG(err);
                switch (err) {
                case WSAEINPROGRESS:
                ...
                case WSAENETRESET:
                    if(type == Socket::TcpSocket) {
                        ...
                        setError(Socket::NetworkError, "the time to live has expired.");
                        // need not close()
                    }
                    return -1;
                case WSANOTINITIALISED:
            ...
            ret = static_cast<int32_t>(bytesRead);
        ...
        if (ret > 0) {
            ...
            ngDebug() << "SocketPrivate::recvfrom(%p \"%s\", %i, %s, %i) == %i",
                   data, qt_prettyDebug(data, min<int32_t>(ret, 16), ret).get(), size,
                   printSender ? addr->toString().get() : "(unknown)",
...
bool SocketPrivate::setTcpKeepalive(bool keepalve, int keepaliveTimeoutSesc, int keepaliveIntervalSesc)
{
    if (!setOption(Socket::KeepAliveOption, keepalve ? 1 : 0)) {
        ngDebug() << "failed to set SO_KEEPALIVE on fd" << fd << "errno:" << WSAGetLastError();
        return false;
    }
    ...
    if (WSAIoctl(fd, SIO_KEEPALIVE_VALS, (LPVOID)&vals, sizeof(vals), NULL, 0, &dummy, NULL, NULL) != 0) {
        ngDebug() << "failed to set SIO_KEEPALIVE_VALS on fd" << fd << "errno:" << WSAGetLastError();
        return false;
    }
#endif
    return true;
...
bool SocketPrivate::setOption(Socket::SocketOption option, int value)
{
    ...
    int v = value;
    if (::setsockopt(static_cast<SOCKET>(fd), level, n, reinterpret_cast<char *>(&v), sizeof(v)) != 0) {
        WS_ERROR_DEBUG(WSAGetLastError());
        return false;
    }
    return true;
                                      ...
                                      const NetworkInterface &iface)
{
    ...
    if (groupAddress.protocol() == HostAddress::IPv6Protocol) {
        ...
        if (iface.isValid()) {
            const vector<NetworkAddressEntry> &addressEntries = iface.addressEntries();
            bool found = false;
...
Socket *SocketPrivate::accept()
{
    ...
    while (true) {
        ...
        if (acceptedDescriptor == static_cast<SOCKET>(SOCKET_ERROR)) {
            ...
            Socket *conn = new Socket(static_cast<intptr_t>(acceptedDescriptor));
            return conn;
        }
        if (!watcher.start()) {
            setError(Socket::UnknownSocketError, UnknownSocketErrorString);
